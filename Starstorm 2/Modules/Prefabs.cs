...
namespace Starstorm2.Modules
{
    ...
    internal static class Prefabs
    {
        ...
        internal static SurvivorDef RegisterNewSurvivor(GameObject bodyPrefab, GameObject displayPrefab, Color charColor, string namePrefix) { return RegisterNewSurvivor(bodyPrefab, displayPrefab, charColor, namePrefix, null, 100f); }

        internal static SurvivorDef RegisterNewSurvivor(GameObject bodyPrefab, GameObject displayPrefab, Color charColor, string namePrefix, float sortPosition) { return RegisterNewSurvivor(bodyPrefab, displayPrefab, charColor, namePrefix, null, sortPosition); }

        internal static SurvivorDef RegisterNewSurvivor(GameObject bodyPrefab, GameObject displayPrefab, Color charColor, string namePrefix, UnlockableDef unlockableDef) { return RegisterNewSurvivor(bodyPrefab, displayPrefab, charColor, namePrefix, unlockableDef, 80f); }

        internal static GameObject CreateDisplayPrefab(string modelName, GameObject prefab, StarstormBodyInfo bodyInfo)
        {
            if (!LegacyResourcesAPI.Load<GameObject>("Prefabs/CharacterBodies/" + bodyInfo.bodyNameToClone + "Body"))
            {
                Debug.LogError(bodyInfo.bodyNameToClone + "Body is not a valid body, character creation failed");
                return null;
            }

            GameObject newPrefab = PrefabAPI.InstantiateClone(LegacyResourcesAPI.Load<GameObject>("Prefabs/CharacterBodies/" + bodyInfo.bodyNameToClone + "Body"), modelName + "Prefab");
        ...
        internal static GameObject CreatePrefab(string bodyName, string modelName, StarstormBodyInfo bodyInfo)
        {
            if (!LegacyResourcesAPI.Load<GameObject>("Prefabs/CharacterBodies/" + bodyInfo.bodyNameToClone + "Body"))
            {
                Debug.LogError(bodyInfo.bodyNameToClone + "Body is not a valid body, character creation failed");
            ...
            GameObject newPrefab = PrefabAPI.InstantiateClone(LegacyResourcesAPI.Load<GameObject>("Prefabs/CharacterBodies/" + bodyInfo.bodyNameToClone + "Body"), bodyName);
            ...
            if (bodyInfo.cameraParams != null) newPrefab.GetComponent<CameraTargetParams>().cameraParams = bodyInfo.cameraParams;
        ...
        #region ModelSetup
        private static Transform SetupModel(GameObject prefab, Transform modelTransform, StarstormBodyInfo bodyInfo)
        {
        ...
        private static GameObject CreateModel(GameObject main, string modelName)
        {
            ...
            GameObject newModel = GameObject.Instantiate(Modules.Assets.mainAssetBundle.LoadAsset<GameObject>(modelName));
            Modules.Assets.ConvertAllRenderersToHopooShader(newModel);
        ...
        internal static void SetupCharacterModel(GameObject prefab, CustomRendererInfo[] rendererInfo, int mainRendererIndex)
        {
            ...
            {
                Debug.LogError("Invalid mainRendererIndex- failed to set mainSkinnedMeshRenderer for " + prefab.name);
                return;
            }
