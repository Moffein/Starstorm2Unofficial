...
namespace Starstorm2Unofficial.Cores.Items
{
    ...
    {
        public override string NameInternal => "SprintHeal";
        public override string Name => "Dormant Fungus";
        public override string Pickup => "Regenerate health while sprinting.";
        public override string Description => $"<style=cIsHealing>Heal {(StaticValues.dungusBase + StaticValues.dungusStack) * 100}%</style> <style=cStack>(+{StaticValues.dungusStack * 100f}% per stack)</style> every second while sprinting.";
    ...
    public class DormantFungusBehavior : CharacterBody.ItemBehavior
    {
        ...
        public void FixedUpdate()
        {
            if (body.isSprinting)
            {
                ...
                if (timer >= StaticValues.dungusTime)
                {
                    manager.HealFractionAuthority(StaticValues.dungusBase + (StaticValues.dungusStack * stack));
                    timer = 0;
                }
            }
            else
