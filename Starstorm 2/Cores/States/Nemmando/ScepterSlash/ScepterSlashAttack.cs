...
namespace Starstorm2.Cores.States.Nemmando
{
    public class ScepterSlashAttack : BaseCustomSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            EffectManager.SpawnEffect(Modules.Assets.nemChargedSlashStartFX, this.attackEffect, true);
            ...
            foreach (HurtBox i in this.targetList)
            {
                if (i)
                {
                    ...
                    if (hurtbox && hurtbox.healthComponent.alive)
                    {
                        ...
                        EffectManager.SpawnEffect(Modules.Assets.nemScepterImpactFX, effect, false);
                    }
                }
            }
        }
            ...
            if (NetworkServer.active)
            {
                foreach (HurtBox i in this.targetList)
                {
                    if (i.healthComponent != this.healthComponent)
                    {
                        ...
                        {
                            Modules.Orbs.NemmandoDashOrb dashOrb = new Modules.Orbs.NemmandoDashOrb();
